...
function showInstructions(){
  ...
  background.init(gameArea.canvas.width, gameArea.canvas.height, 'Pictures/bg1.png', 0, 0, 'image', WALKING, true);
...
function startLevel() {
  ...
  background.init(gameArea.canvas.width, gameArea.canvas.height, `Pictures/bg1.png`, -50, 0, 'image', WALKING);
  background2.init(gameArea.canvas.width, gameArea.canvas.height, `Pictures/bg1_balik.png`,850, 0, 'image', WALKING);
  ...
  this.init = function(width, height, color, x, y, dataType, moveType, initialShow = false, charName = undefined) {
    ...
    if (dataType === 'image') {
      ...
      if (charName) {
        ...
        this.imageMirror.src = `Pictures/${charName}_back.png`;
...
function updateGameArea() {
  ...
  for (let i = 0; i < enemyCharacters.length; i++) {
    if (enemyCharacters[i].isAlive()) {
      ...
      //This tells  enemies whether to go up or down
      ...
      //This rotates enemies of type 2--currently just the js enemies 
      if (enemyCharacters[i].moveType === ROTATING) {
        enemyCharacters[i].angle += 10 * Math.PI / 180;
      }
    } else { // if dead; enemy will be 'squeezed', fall to the ground and fade away. 
